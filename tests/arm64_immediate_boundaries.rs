...
fn memory_immediates_preserve_addressing_mode_boundaries() {
    for (source, expected) in [
        ...
        ("ldrb w0, [x1, #-1]", 0x385F_F020),
        ("ldrh w0, [x1, #3]", 0x7840_3020),
        ("strb w0, [x1, #-1]", 0x381F_F020),
        ("strh w0, [x1, #3]", 0x7800_3020),
        ("ldrsb w0, [x1, #-1]", 0x38DF_F020),
        ("ldrsb x0, [x1, #-1]", 0x389F_F020),
        ("ldrsh w0, [x1, #3]", 0x78C0_3020),
        ("ldrsh x0, [x1, #3]", 0x7880_3020),
        ("ldrsw x0, [x1, #3]", 0xB880_3020),
    ] {
        assert_encoding(source, expected);
    }
}
